...
public class STStore {
    
    public typealias Complition<T> = (_ result: T) -> Void
    
    static public let store = STStore()
    private var productsRequests = [ProductsRequest]()
    private let purchese = Purchese()
    
    private init() {}
    
    public func products(by identifiers: [String], success: @escaping Complition<[Product]>, failure: @escaping Complition<StoreError>) {
    ...
    public func buy(product: SKProduct, success: @escaping Complition<SKPaymentTransaction>, failure: @escaping Complition<StoreError>) {
        self.purchese.buy(product: product, success: success, failure: failure)
    }
    
    public func buy(product: Product, success: @escaping Complition<SKPaymentTransaction>, failure: @escaping Complition<StoreError>) {
