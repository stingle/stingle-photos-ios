...
public struct STHeaders {
    ...
    public var thumb: STHeader?
}

public struct STHeader: Equatable {
...
extension STCrypto {
    ...
    func getFileHeader(input: InputStream, publicKey: Bytes? = nil, privateKey: Bytes? = nil) throws -> STHeader {
        ...
        // The sealed box gives confidentiality + ciphertext integrity, but NOT sender authenticity:
        // anyone who knows the recipient public key can forge a header that opens successfully.
        // The decrypted fields below are therefore attacker-controlled and every slice must be
        // bounds-checked against the actual decrypted length, otherwise a forged/short header traps.
        offset = 0
        // Throws (rather than fatally trapping) if `length` bytes are not available at `offset`.
        func take(_ length: Int) throws -> ArraySlice<UInt8> {
            guard length >= 0, offset <= headerBytes.count, length <= headerBytes.count - offset else {
                throw CryptoError.Header.incorrectHeaderSize
            }
            let slice = headerBytes[offset..<offset + length]
            offset += length
            return slice
        }

        header.headerVersion = try take(Constants.HeaderVersionLen).first ?? 0

        header.chunkSize = STCrypto.fromBytes(b: Bytes(try take(Constants.FileChunksizeLen)))
        header.dataSize = STCrypto.fromBytes(b: Bytes(try take(Constants.FileDataSizeLen)))

        // Validate ranges centrally so every downstream consumer (incl. the player path, which does
        // not re-validate) inherits the guarantee: a 0 chunkSize would divide-by-zero in the player,
        // and a dataSize > Int64.max traps on the off_t conversions used during decryption.
        guard header.chunkSize >= 1, header.chunkSize < UInt32(Constants.MAX_BUFFER_LENGTH) else {
            throw CryptoError.Header.incorrectChunkSize
        }
        guard header.dataSize <= UInt64(Int64.max) else {
            throw CryptoError.Header.incorrectHeaderSize
        }

        header.symmetricKey = Bytes(try take(sodium.keyDerivation.KeyBytes))

        header.fileType = try take(Constants.FileTypeLen).first ?? 0

        let fileNameSize:Int = STCrypto.fromBytes(b: Bytes(try take(Constants.FileNameSizeLen)))
        header.fileName = String(bytes: Bytes(try take(fileNameSize)), encoding: String.Encoding.utf8) ?? ""
        header.videoDuration = STCrypto.fromBytes(b: Bytes(try take(Constants.FileVideoDurationlen)))
        return header
    }
