...
public struct STDarwinNotification {
...
public final class STDarwinNotificationCenter {
    ...
    fileprivate final class Observation {
        ...
        let name: STDarwinNotification.Name

        /// The interested object
        weak var observer: AnyObject?

        init(observer: AnyObject, name: STDarwinNotification.Name, handler: @escaping NotificationHandler) {
    ...
    public typealias NotificationHandler = ((STDarwinNotification) -> Void)
    
    /// The shared DarwinNotificationCenter, it will always return the same instance.
    public static var shared = STDarwinNotificationCenter()
    ...
    public func addObserver(_ observer: AnyObject, for name: STDarwinNotification.Name, using handler: @escaping NotificationHandler) {
        cleanupObservers()
        queue.async {
    ...
    public func removeObserver(_ observer: AnyObject, for name: STDarwinNotification.Name? = nil) {
        cleanupObservers()
    ...
    public func isObserver(_ observer: AnyObject, for name: STDarwinNotification.Name? = nil) -> Bool {
        cleanupObservers()
    ...
    public func postNotification(_ name: STDarwinNotification.Name) {
    ...
    private func signalNotification(_ name: STDarwinNotification.Name) {
        cleanupObservers()
        queue.async {
            let affectedObservations = self.observations.filter({ (observation) -> Bool in
                return observation.name == name
            })
            let notification = STDarwinNotification(name)
...
// MARK: -
extension STDarwinNotificationCenter.Observation: Equatable {
    
    /// Start observing the notification.
    fileprivate func observe() {
        guard let cfCenter = STDarwinNotificationCenter.shared.center else {
        ...
        let callback: CFNotificationCallback = { (center, observer, name, object, userInfo) in
            ...
            let notificationName = STDarwinNotification.Name(cfName)
            STDarwinNotificationCenter.shared.signalNotification(notificationName)
        }
