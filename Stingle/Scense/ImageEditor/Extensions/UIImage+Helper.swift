...
extension UIImage {
    ...
    private class func maxWidthForCropSize(_ cropSize: CGSize) -> CGFloat {
        var maxWidth = Config.croppingImageShortSideMaxSize
        if cropSize.width > cropSize.height {
            maxWidth = Config.croppingImageShortSideMaxSize * cropSize.width / cropSize.height
            if maxWidth > Config.croppingImageLongSideMaxSize {
                maxWidth = Config.croppingImageLongSideMaxSize
            }
        } else {
            maxWidth = Config.croppingImageShortSideMaxSize
            let height = maxWidth * cropSize.height / cropSize.width
            if height > Config.croppingImageLongSideMaxSize {
                maxWidth = Config.croppingImageLongSideMaxSize * cropSize.width / cropSize.height
            }
        }
        return maxWidth
    }
