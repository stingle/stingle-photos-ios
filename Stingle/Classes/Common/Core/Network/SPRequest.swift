...
struct SPUploadFileRequest : SPRequest {
	...
	init(file:SPFileInfo, folder:Int) {
        token = STApplication.shared.utils.user()!.token
...
struct SPTrashFilesRequest : SPRequest {
	...
	func bodyParams () -> String {
		...
		guard let encParams = try? crypto.encryptParamsForServer(params: params) else {
			return ""
		}
		return encParams
	}
...
struct SPRestoreFilesRequest : SPRequest {
	...
	func bodyParams () -> String {
        ...
        guard let encParams = try? STApplication.shared.crypto.encryptParamsForServer(params: params) else {
			return ""
		}
		return encParams
	}
