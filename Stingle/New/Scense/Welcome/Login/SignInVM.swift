...
class SignInVM {
	...
	func login(email: String?, password: String?, success: @escaping ((_ result: STUser) -> Void), failure: @escaping ((_ error: IError) -> Void)) {
		do {
	...
	private func login(email: String, password: String, success: @escaping ((_ result: STUser) -> Void), failure: @escaping ((_ error: IError) -> Void)) {
