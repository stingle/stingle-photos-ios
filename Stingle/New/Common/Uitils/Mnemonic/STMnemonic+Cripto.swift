...
extension STMnemonic {
    ...
    func toBitArray(data: Data) -> [String] {
        var toReturn = [String]()
        for num: UInt8 in data.bytes {
            toReturn.append(contentsOf: self.mnemonicBits(integer: num))
        }
        return toReturn
    }
    
    func bytes(data: Data) -> Array<UInt8> {
        return  Array(data)
    }
    
    func mnemonicBits(integer: UInt8) -> [String] {
        let totalBitsCount = MemoryLayout<UInt8>.size * 8
        var bitsArray = [String](repeating: "0", count: totalBitsCount)
        for j in 0 ..< totalBitsCount {
            let bitVal: UInt8 = 1 << UInt8(totalBitsCount - 1 - j)
            let check = integer & bitVal
    ...
    func mnemonicData(string: String) -> Data {
