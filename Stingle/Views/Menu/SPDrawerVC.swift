...
class DrawerController: UIViewController, RootViewControllerDelegate {
	...
	var menuController: SPMenuVC
	var isMenuExpanded: Bool = false
	let overlayView = UIView()
	
	init(rootViewController: RootViewController, menuController: SPMenuVC) {
	...
	func toggleMenu() {
		...
		UIView.animate(withDuration: 0.3, animations: {
			...
			self.menuController .hideChilds(shouldHide: !self.isMenuExpanded)
